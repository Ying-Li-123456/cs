...
/**
 * The main edited part is the copy_unary_op and copy_expr part.
 */
#include "iobuffer.h"
#include "symbols.h"
...
ast copy_expr(ast t)
{
    ...
    for ( int i = 0 ; i < size ; i++ )
    {
        ...
        // According to the description in MyUni:
        // case 1: term & 0 => false
        if (i % 2 == 0 && i >=2)
        {
            ast term1 = get_expr(t,i-1) ;
            string str1 = get_infix_op_op(term1) ;
            ast term2 = get_expr(t,i) ;
            ast term_kind = get_term_term(term2) ;
            ast kind = ast_node_kind(term_kind) ;
            
            if (str1 == "&" && kind == ast_int)
            {
                ast expr = get_expr(t,i) ;
                ast integer = get_term_term(expr) ;
                int number = get_int_constant(integer) ;

                if(number == 0)
                {
                    // delete the redundant nodes and return false
                    for (int j = terms.size(); j > 0 ; j--)
                    {
                        terms.pop_back() ;
                    }
                    
                    ast f = create_bool(false) ;
                    copy = create_term(f) ;
                    copied = true ;
                }
            }
        ...
        // According to the description in MyUni:
        // case 2: variable < variable => false (x<x => false)
        //         variable > variable => false (x>x => false)
        if(i%2 == 0 && i>=2)
        {
            ast expr1 = get_expr(t,i-1) ;
            string str1 = get_infix_op_op(expr1) ;
        
            if(str1 == "<" || str1 == ">")
            {
                ast expr2 = get_expr(t,i) ;
                ast term_kind2 = get_term_term(expr2) ;
                ast kind = ast_node_kind(term_kind2) ;
            
                if(kind == ast_var)
                {
                    ast expr3 = get_expr(t,i-2) ;
                    ast term_kind3 = get_term_term(expr3) ;
                    string name2 = get_var_name(term_kind2) ;
                    string name3 = get_var_name(term_kind3) ;
        ...
        // According to the description in MyUni:
        // case 3: variable & variable => variable (a&a => a)
        //         variable | variable => variable (a|a => a)
        if(i%2 == 0 && i>=2)
        {
            ast term1 = get_expr(t,i-1) ;
            string str1 = get_infix_op_op(term1) ;
            ...
            if(str1 == "&" || str1 == "|")
            {
                ast expr1 = get_expr(t,i) ;
                ast term_kind1 = get_term_term(expr1) ;
                ast kind = ast_node_kind(term_kind1) ;
                
                if(kind == ast_var)
                {
                    ast expr2 = get_expr(t,i-2) ;
                    ast term_kind2 = get_term_term(expr2) ;
                    string name1 = get_var_name(term_kind1) ;
                    string name2 = get_var_name(term_kind2) ;
    ...
    // all operations in an expr
    for(int i=terms.size()-2;i>=1;i=i-2)
    {
        // get the term before and after the operation and check whether
        // they are integer or not. It's only the integers that we need to compute.
        ast term_kind1 = get_term_term(terms.at(i-1)) ;
        ast kind_before = ast_node_kind(term_kind1) ;
        ast term_kind2 = get_term_term(terms.at(i+1)) ;
        ast kind_after = ast_node_kind(term_kind2) ;
        
        if(kind_before == ast_int && kind_after == ast_int)
        {
            string operation = get_infix_op_op(terms[i]) ;
            int number_before = get_int_constant(term_kind1) ;
            int number_after = get_int_constant(term_kind2) ;
            int result ;
            ...
            ast int_res = create_int(result) ;
            ast res = create_term(int_res) ;
            int after = i+1 ;
            int before = i-1 ;
            
            // delete the before number, after number and the operation
            // must be deleted in order: from the back forward
            // store the result computing above in vector
            terms.erase(terms.begin()+after) ;
            terms.erase(terms.begin()+i) ;
            terms.erase(terms.begin()+before) ;
            terms.insert(terms.begin()+before,res) ;
            copied = true ;
        }
    }
...
ast copy_term(ast t)
{
    ...
    switch(ast_node_kind(term))
    {
        ...
        case ast_expr:
            ...
            // According to the rules described in myUni: delete the redudant term nodes,
            // just simply replace all redundant nodes with the inner one.
            if(size_of_expr(copy)==1)
            {
                ast expr2 = get_expr(copy,0) ;
                ast term2 = get_term_term(expr2) ;
                copy = term2 ;
                break;
            }
...
// deal with the unary operation
ast copy_unary_op(ast t)
{
    string uop = get_unary_op_op(t);
    ast term = get_unary_op_term(t) ;
    ast copy = copy_term(term) ;
    ast term1 = get_term_term(term);
    ast kind = ast_node_kind(term1);
    
    // case: --3 => 3
    if(kind == ast_unary_op)
    {
        string unary_operation = get_unary_op_op(term1);

        if(uop == unary_operation)
        {
            ast op_term = get_unary_op_term(term1);

            return get_term_term(op_term);
        }
    }
    else if(kind == ast_int)
    {
        int number = get_int_constant(term1);
