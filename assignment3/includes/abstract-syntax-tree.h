...
{
    // we use integers to identify AST and Annotation nodes in the public interface
    typedef int ast ;
    typedef int ann ;
    ...
    enum ast_kind
    {
        ...
        ast_annotation,         // an annotation node
    ...
    */
    ...
    // create a new empty annotation object
    ...
    // comments vector
    extern ann add_ann_comments(ann a,string s) ;
    extern ann delete_ann_comments(ann a,int index) ;
    extern int size_of_ann_comments(ann a) ;
    extern string get_ann_comments(ann a,int index) ;

    // warnings vector
    extern ann add_ann_warnings(ann a,string s) ;
    extern ann delete_ann_warnings(ann a,int index) ;
    extern int size_of_ann_warnings(ann a) ;
    extern string get_ann_warnings(ann a,int index) ;

    // errors vector
    extern ann add_ann_errors(ann a,string s) ;
    extern ann delete_ann_errors(ann a,int index) ;
    extern int size_of_ann_errors(ann a) ;
    ...
    // it will return a size of 0 if a size_of_*() function is applied to it
    ...
    extern int size_of_class_var_decs(ast t) ;
    ...
    extern int size_of_var_decs(ast t) ;
    ...
    extern int size_of_subr_decs(ast t) ;
    ...
    // params must be an ast_param_list or ast_empty node
    // body must be an ast_subr_body or ast_empty node
    ...
    // decs must be an ast_var_decs or ast_empty node
    ...
    // expr_list must be an ast_expr_list or ast_empty node
